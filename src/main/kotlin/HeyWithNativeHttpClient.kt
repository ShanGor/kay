...
fun heyWithNativeHttpClient(args: Array<String>) {
    ArgParser(args).parseInto(::HeyArgs).run {
        ...
        val client = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(timeout)).build()
        ...
        GlobalScope.launch(Dispatchers.Default.limitedParallelism(1)) {
            repeat(number) {
                ...
                client.sendAsync(currentRequest, BodyHandlers.ofString())
                    .whenComplete { resp, e ->
                        try {
                            val status =
                                if (e != null) {
